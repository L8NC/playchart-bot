...
// event. On each, we check the message's poll.resultsFinalized state.
//
// We track which polls we've already posted results for so we don't
// announce twice.

import type { Client, PollData } from 'discord.js'
...
export function registerPollEndedListener(client: Client): void {
  ...
  client.on(Events.MessagePollVoteAdd, async (answer) => {
    await maybePostResult(answer.poll.message.id, answer.poll, answer.poll.message)
  })
...
async function maybePostResult(
  messageId: string,
  poll: PollData,
  message: unknown,
): Promise<void> {
  ...
  try {
    await postPollResult(message as Parameters<typeof postPollResult>[0])
  } catch (err) {
